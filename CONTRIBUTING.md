...
1. 変更した内容を `master` ブランチにマージし、本番サーバ `/var/www/home.tcu-ctrl.jp` で `git pull --ff-only` → `yarn install --frozen-lockfile` → `yarn build` のあと `sudo systemctl restart home-tcu-ctrl` すると、実際のWebページに反映されます。
